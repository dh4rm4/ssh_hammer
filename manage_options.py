...
class user_options():
	...
	if (self.passwd_file != None and
	    self.is_valid_file(self.passwd_file) == False) or \
	    (self.cmd_file != None and
	     self.is_valid_file(self.cmd_file) == False):
	    exit()
	if (self.user_gave_all_necessary_options() == False):
	    print(self.parser.usage)
	    exit()

    def user_gave_all_necessary_options(self):
	if (self.host == None or self.user == None or self.passwd_file == None):
	    return False
	return True
